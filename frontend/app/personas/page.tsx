    ...
    fetchPersonas()
      ...
      .catch((err: unknown) => {
        if (active)
          setState({
            kind: "error",
            message: err instanceof Error ? err.message : "Failed to load personas",
          });
      });
    return () => {
      active = false;
...
const styles: Record<string, React.CSSProperties> = {
  main: { maxWidth: 880, margin: "0 auto", padding: "32px 24px 80px", lineHeight: 1.5 },
  ...
  primary: { padding: "9px 16px", background: "#111827", color: "white", borderRadius: 6, fontSize: 14, whiteSpace: "nowrap" },
  muted: { color: "#6b7280" },
  error: { color: "#991b1b", background: "#fef2f2", padding: "10px 12px", borderRadius: 6 },
  empty: { textAlign: "center", padding: "56px 24px", border: "1px dashed #d1d5db", borderRadius: 12, marginTop: 12 },
  emptyTitle: { margin: "0 0 8px", fontSize: 20 },
  grid: { listStyle: "none", padding: 0, margin: 0, display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: 16 },
  card: { border: "1px solid #e5e7eb", borderRadius: 12, overflow: "hidden", background: "white" },
  cardLink: { display: "block", padding: 18, color: "inherit", textDecoration: "none" },
  cardTitle: { margin: "0 0 6px", fontSize: 18 },
  cardVoice: { margin: "0 0 8px", color: "#374151", fontSize: 14 },
  cardSample: { margin: "0 0 12px", color: "#6b7280", fontStyle: "italic", fontSize: 13 },
  chips: { display: "flex", flexWrap: "wrap", gap: 6 },
  chip: { fontSize: 12, background: "#f3f4f6", borderRadius: 999, padding: "2px 10px", color: "#374151" },
};
