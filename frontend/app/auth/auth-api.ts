 ...
 *
 * The `AuthUser` shape mirrors the documented backend contract
 * `{ id, email, role, createdAt }`. It is declared locally (not imported from
 * @publisher/shared) so this frontend slice stays self-contained.
 */
...
export async function fetchMe(
  ...
  if (!res.ok) {
    throw new Error(await readError(res, `Session expired (HTTP ${res.status})`));
  }
